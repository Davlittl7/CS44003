...
class GameSceneViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        configVC = self.tabBarController!.viewControllers![1] as? ConfigSceneViewController
    }
    //================================================
...
//================================================
    ...
    private func createRectangle() -> UIButton {
        ...
        //Button/rectangle setup using connectivity from config scene
        if(configVC?.randOrBlueSwitch != nil) {
            if(configVC?.randOrBlueSwitch.isOn == true) {
...
extension GameSceneViewController {
    ...
    private func updateHighScores() {
        let highScoreSize = highScores.count - 1
        for i in highScores.indices {
            if(highScores[highScoreSize + i] < rectPairsTouched) {
                highScores[highScoreSize + i] = rectPairsTouched
                return
            }
    ...
    private func stopGameRunning() {
        ...
        //Makes sure to update our scores once the game is over
        //And sorts them for largest to smallest for the table view
        updateHighScores()
        highScores.sort(by: >)
    }
